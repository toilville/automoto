...
| Tool | Description |
|------|-------------|
| `search_research` | Search all MSR content — publications, researchers, projects, news |
| `find_researchers` | Find researchers by name, topic, lab, or area |
| `browse_publications` | Browse publications by topic, author, or date |
| `get_research_areas` | List all research areas and labs |
| `get_latest_news` | Fetch recent news and blog posts |
| `search_rag_collection` | Semantic search within MSR RAG knowledge base |
...
|----------|-----|-------------|
| RAG Collections | `msr://rag/collections` | All RAG knowledge collections |
| RAG Sources | `msr://rag/sources` | All RAG data sources |
| Research Area | `msr://research-areas/{areaId}` | Details for a specific research area |
...
| `find-expert` | Find the right MSR expert for a topic |
...
| `MCP_AUTH_AUDIENCE` | Yes | App registration audience (e.g., `api://msr-mcp-server`) |
...
MCP_AUTH_AUDIENCE=api://msr-mcp-server \
...
- **Teams channel**: MSR Event Agent team
