...
function buildProviders(): IAnalyticsProvider[] {
  ...
  if (mode === "1ds" || mode === "both") {
    // autoInit: false assumes the 1DS SDK is loaded via <script> tag
    // and attached to window.analytics already.
    // Set autoInit: true if you want the provider to bootstrap the SDK.
