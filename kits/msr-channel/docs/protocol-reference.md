...
interface ChannelAdapter<TInbound, TOutbound, TStreamChunk> {
  ...
  /** Normalize inbound platform-native request → canonical MSRAgentRequest */
  pub(raw: TInbound): MSRAgentRequest;

  /** Format canonical MSRAgentResponse → platform-native outbound */
  sub(response: MSRAgentResponse): TOutbound;

  /** Format MSRStreamEvent → platform-native streaming chunk (null to skip) */
...
Your platform sends a message. The adapter normalizes it into a canonical `MSRAgentRequest`.
...
interface MSRContext {
  ...
  conversation?: { id: string; history: MSRMessage[] };
...
export class CLIAdapter implements ChannelAdapter<CLIInbound, CLIOutbound, null> {
  ...
  pub(raw: CLIInbound): MSRAgentRequest {
    return {
      type: "chat",
  ...
  sub(response: MSRAgentResponse): CLIOutbound {
...
stream(event: MSRStreamEvent): string | null {
  switch (event.type) {
    case "text_delta":
