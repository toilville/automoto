...
| Method | Purpose |
|--------|---------|
| `pub(native)` | Normalize your platform's inbound request → canonical `MSRAgentRequest` |
| `sub(response)` | Format canonical `MSRAgentResponse` → your platform's native response |
| `stream(event)` | Format `MSRStreamEvent` → your platform's streaming chunk |
...
```
Your Platform ──webhook──→ /api/messages ──pub()──→ MSRAgentRequest
                                                        │
                                                   Agent Backend
                                                        │
Your Platform ←─response── /api/messages ←─sub()──── MSRAgentResponse
```
